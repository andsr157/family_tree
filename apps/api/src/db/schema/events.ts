...
export const events = pgTable(
  'events',
  {
    ...
    date: date('date'),
    dateText: varchar('date_text', { length: 100 }),
    dateQualifier: varchar('date_qualifier', { length: 20 }).notNull().default('exact'),
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.dateQualifier} IN ('exact','about','before','after','between')`,
    ),
  ],
)
